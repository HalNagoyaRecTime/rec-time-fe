...
export type EventRow = {
    ...
    f_entries?: string[] | null; // B) 출전 학번 목록 出走学番の一覧
};
...
export type UpdateRow = {
    update_id: string;
    event_id?: string | null;
    field: string; // 변경된 필드명 (예: "f_start_time") 変更されたフィールド名(例: "f_start_time")
    old_value: any;
    new_value: any;
    at_iso: string; // ISO 시각 ISO時刻
...
export function fetchStudent(id: string | null = getStudentId()): StudentRow | null {
    if (!id) return null;
    return readJSON<StudentRow>(LS_KEY_STUDENT(id), null as any);
}

//// ② 이벤트 정보 (+출전 여부) イベント情報(+出場可否)
export function fetchEvents(id: string | null = getStudentId()): EventRow[] {
    if (!id) return [];
    return readJSON<EventRow[]>(LS_KEY_EVENTS(id), []);
}
/** 내가 출전하는 경기인지 확인 自分が出場する試合なのか確認 */
export function isMyEntry(ev: EventRow, studentId: string | null = getStudentId()): boolean {
...
/** 출전 여부에 따른 색상 클래스 (예시)  出場有無によるカラークラス(例)*/
export function getEventColorClass(ev: EventRow, studentId: string | null = getStudentId()): string {
...
//// ④ 직후 출전 이벤트 直後の出場イベント
export function getNextMyEvent(id: string | null = getStudentId()): EventRow | null {
...
//// ⑤ 알림 기록 (아직 저장은 안 함, 읽기만 가능) 通知記録(まだ保存はしていない、読み込みのみ)
export function fetchNotifHistory(id: string | null = getStudentId()): NotifRow[] {
    if (!id) return [];
    return readJSON<NotifRow[]>(LS_KEY_NOTIFS(id), []);
}

//// ⑥ 변경 기록 (아직 저장은 안 함, 읽기만 가능) 変更記録(まだ保存はしていない、読み込みのみ)
export function fetchUpdateHistory(id: string | null = getStudentId()): UpdateRow[] {
    if (!id) return [];
